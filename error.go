...
func (e ErrorObject) AddParam(name string, value interface{}) Error {
	if e.ErrParams == nil {
		e.ErrParams = make(map[string]interface{})
	}

	e.ErrParams[name] = value
	return e
}
...
func (e ErrorObject) Error() string {
	if len(e.ErrParams) == 0 {
		return e.ErrMessage
	}

	res := bytes.Buffer{}
	_ = template.Must(template.New("err").Parse(e.ErrMessage)).Execute(&res, e.ErrParams)
