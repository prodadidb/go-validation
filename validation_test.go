...
func TestValidate(t *testing.T) {
	...
	for _, test := range tests {
		err := validation.Validate(test.value)
		assertError(t, test.err, err, test.tag)
		// rules that are not context-aware should still be applied in context-aware validation
		err = validation.ValidateWithContext(context.Background(), test.value)
		assertError(t, test.errWithContext, err, test.tag)
	}

	// with rules
	err := validation.Validate("123", &validateAbc{}, &validateXyz{})
	assert.EqualError(t, err, "error abc")
	err = validation.Validate("abc", &validateAbc{}, &validateXyz{})
	assert.EqualError(t, err, "error xyz")
	err = validation.Validate("abcxyz", &validateAbc{}, &validateXyz{})
	assert.NoError(t, err)

	err = validation.Validate("123", &validateAbc{}, validation.Skip, &validateXyz{})
	assert.EqualError(t, err, "error abc")
	err = validation.Validate("abc", &validateAbc{}, validation.Skip, &validateXyz{})
	assert.NoError(t, err)

	err = validation.Validate("123", &validateAbc{}, validation.Skip.When(true), &validateXyz{})
	assert.EqualError(t, err, "error abc")
	err = validation.Validate("abc", &validateAbc{}, validation.Skip.When(true), &validateXyz{})
	assert.NoError(t, err)

	err = validation.Validate("123", &validateAbc{}, validation.Skip.When(false), &validateXyz{})
	assert.EqualError(t, err, "error abc")
	err = validation.Validate("abc", &validateAbc{}, validation.Skip.When(false), &validateXyz{})
	assert.EqualError(t, err, "error xyz")
}

func stringEqual(str string) validation.RuleFunc {
...
func TestBy(t *testing.T) {
	abcRule := validation.By(func(value interface{}) error {
		s, _ := value.(string)
		if s != "abc" {
	...
	assert.Nil(t, validation.Validate("abc", abcRule))
	err := validation.Validate("xyz", abcRule)
	if assert.NotNil(t, err) {
		assert.Equal(t, "must be abc", err.Error())
	}

	xyzRule := validation.By(stringEqual("xyz"))
	assert.Nil(t, validation.Validate("xyz", xyzRule))
	assert.NotNil(t, validation.Validate("abc", xyzRule))
	assert.Nil(t, validation.ValidateWithContext(context.Background(), "xyz", xyzRule))
	assert.NotNil(t, validation.ValidateWithContext(context.Background(), "abc", xyzRule))
}
...
func TestByWithContext(t *testing.T) {
	k := key(1)
	abcRule := validation.WithContext(func(ctx context.Context, value interface{}) error {
	...
	assert.Nil(t, validation.ValidateWithContext(ctx, "abc", abcRule))
	err := validation.ValidateWithContext(ctx, "xyz", abcRule)
	if assert.NotNil(t, err) {
		assert.Equal(t, "must be abc", err.Error())
	}

	assert.NotNil(t, validation.Validate("abc", abcRule))
}

func Test_skipRule_Validate(t *testing.T) {
	assert.Nil(t, validation.Skip.Validate(100))
}
...
func (v *validateInternalError) Validate(obj interface{}) error {
	if strings.Contains(obj.(string), "internal") {
		return validation.NewInternalError(errors.New("error internal"))
	}
	return nil
}
...
func (m Model3) Validate() error {
	return validation.ValidateStruct(&m,
		validation.Field(&m.A, &validateAbc{}),
	)
}
...
func (m Model4) ValidateWithContext(ctx context.Context) error {
	return validation.ValidateStructWithContext(ctx, &m,
		validation.Field(&m.A, &validateContextAbc{}),
	)
}
